...
| Gemma MTP target | `google/gemma-4-12B-it-qat-q4_0-unquantized` @ `b6ed862…` | Google HF | SHA/structure passed; runtime pending | Yes | N/A | Exact target for 12B assistant | Text/image/video/audio | Output-equivalence test required | Q4_0 QAT extracted half precision | Yes, MTP benchmark |
| Gemma assistant | `google/gemma-4-12B-it-qat-q4_0-unquantized-assistant` @ `1893406…` | Google HF | Exact offline pair passed; runtime pending | Yes | N/A | Dedicated assistant | Follows target path | Output-equivalence test required | Q4_0 QAT assistant | Yes, exact-pair gated |
...
| vLLM MTP fix | commit `b2b8f679d058…`, cu130 wheel | vLLM commit/PR/nightly index | RTX 5090 CUDA smoke passed | Exact-pair test pending | Conditional | Installed source contains EAGLE-only width guard | Regression test required | Regression test required | Exact wheel/package check passed | MTP candidate |
