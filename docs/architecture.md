...
Its Gemma conversation harness owns persona dialogue and proposes only
closed-schema actions and slots. The reservation domain validates and executes
those proposals after explicit confirmation. The Qwen3-TTS adapter projects an
already-final text response onto the existing ordered PCM protocol. Neither
model receives reservation mutation authority.
