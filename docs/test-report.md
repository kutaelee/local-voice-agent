...
| PC-server domain/API/registry/planner/router/adapter unit tests | Passed | Latest run: 75 tests in 3.47 s; prior coverage plus explicit loopback URL enforcement, exact execution/digest/expiry IPC binding, Level 0-only dispatch, and closed executor-response validation |
...
| Tool Executor Windows-native suite | Passed | Latest run: 57 tests in 4.02 s using isolated Python 3.12.13; prior filesystem/Git coverage plus configuration loading, authenticated bounded API, exact digest/expiry binding, duplicate/conflict behavior, and sanitized no-replace evidence |
| Tool Executor WSL suite | Passed | Latest run: same 57 tests in 3.31 s using a separate WSL Python 3.12.13 environment; internal and escaping symlinks rejected and host-specific workspace filtering verified |
