...
| PC-server domain/API/registry/planner/router/adapter/audio unit tests | Passed | Latest run: 119 tests; prior coverage plus the bounded model tool loop, exact approval continuation/rejection, WSL-gateway allowlist, VAD worker/endpoint handling, in-flight cancellation, closed client payloads, Unix worker adapters, and the STT/conversation/TTS voice-turn event sequence |
...
| Tool Executor Windows-native suite | Passed | Latest run: 68 passed and one symlink case skipped because unprivileged Windows cannot create it; prior filesystem/Git coverage plus approved browser/UI integration, approval-bound write/patch/rollback, atomic replacement, concurrent-hash rejection, API binding, and no-replace evidence |
| Tool Executor WSL suite | Passed | Latest run: 67 passed and two Windows-only browser/UI cases skipped using a separate WSL Python 3.12.13 environment; filesystem/Git, internal and escaping symlink mutation rejection, create/replace/patch, concurrent-change rollback rejection, and browser URL policy passed |
...
| PostgreSQL migration and transactional outbox | Passed | Exact PostgreSQL 18.4 image started healthy on `127.0.0.1:55432`; Alembic applied `0001_initial`; 10 domain tables plus `alembic_version` exist. A real asyncpg test verified atomic plan/event/outbox insertion, transition/event/outbox insertion, idempotent replay, conflicting-key rejection, optimistic-lock rejection, reconnect recovery, and test-data cleanup |
...
| Android 0.4.0 artifacts | Passed | Debug APK 12,456,668 bytes SHA-256 `d820ede01e9bae5b9cf238389e3592039317f3b95e6dc332169f249ea08f27ac`; unsigned release APK 8,869,323 bytes SHA-256 `5cae100ab65de32fc4f694c587d65e19ae6e5113579d3d7ff30bbbc6417d62ca`; AAPT2 confirmed version code 4/name 0.4.0 and API 37 |
