...
| Physical Android QA collector | Passed (static, parse, and no-device fail-closed gate) | The registered collector verifies the exact 0.6.9 APK hash, rejects emulators, requires one selected authorized physical device, optionally installs only that APK, and atomically records all 16 physical cases including profile selection/similarity/playback speed without tokens, raw audio, full transcripts, or unrelated logs. A real cold ADB start captured its normal stderr daemon notice without weakening exit-code checks, rejected the empty device list with the intended error in 2,324 ms, and stopped only the QA-started daemon. A separate ownership probe preserved a pre-existing ADB daemon. PowerShell parsing and focused tests pass; structured device cases remain `NOT_RUN` until hardware is attached through ADB. |
...
| Gemma-led conversation harness regression | Passed | Replaced the deterministic intent/response loop with a persona/state/history harness and guarded reservation actions. Full PC suite passed with two environment skips; 6/6 live model action/scope cases passed at 1,109–1,468 ms. Browser replay of the reported sequence produced contextual replies for date, service choices, and persona identity without repeating the caller or the missing-field prompt. TTS remained disabled. |
| Detailed salon menu and prefilled QA schedule | Passed | The policy exposes ten cut/color/perm/clinic services with duration, price, aliases, and qualified staff. The `web-qa` instance uses an isolated table seeded with three fictitious reservations and refuses to seed any other filename. Live browser QA listed all ten services and existing bookings; for the occupied 2026-07-29 14:00 digital-perm slot, Gemma correctly reported only 준 as available. Two unrelated follow-up questions were declined with distinct wording and no internal action/slot leakage. |
